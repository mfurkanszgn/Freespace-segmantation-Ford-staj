...
def tensorize_image(image_path_list, output_shape, cuda=False):
    ...
    for image_path in image_path_list:
        ...
        image = cv2.resize(image,tuple(output_shape))
...
def torchlike_data(data):
   ...
   # print(data)
    ...
    for ch in range(n_channels):
           ...
           # print( torchlike_data_output[ch])
           # print(torchlike_data_output[ch],"->>>", data[:,:,ch],"bla bla \n")
...
def one_hot_encoder(data, n_class):
    ...
    encoded_data = np.zeros((*data.shape, n_class), dtype=np.int64)
    ...
    for lbl in range(n_class):  # kategorize eder. Maskeyi 2 katmanli hale getirir(0 ve 1 olarak). 
    
        #print("data",data,"->>>",lbl)
       
        encoded_data[:,:,lbl][data==lbl]=1 
        #egerki maskenin 0. katmanında(lbl=0 iken) data yani mask resmindeki degerler 0 ise o degerleri 1 yap.
        # egerki maskenin 0. katmanında ki (lbl=0 iken ) data yani maskeli resimdeki pikselde ki deger 1 ise yani etiketli ise orası 0 olarak kalır.
        
         #egerki maskenin 1. katmanında(lbl=1 iken) ,data yani mask resmindeki degerler 0 ise o degerleri 0 yap.
          #egerki maskenin 1. katmanında(lbl=1 iken) data yani mask resmindeki degerler 1 ise o degerleri 1 yap.
